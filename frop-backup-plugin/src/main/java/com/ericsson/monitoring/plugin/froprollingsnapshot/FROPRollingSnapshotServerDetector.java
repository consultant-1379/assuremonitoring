...
public class FROPRollingSnapshotServerDetector extends ServerDetector implements AutoServerDetector {

    public static final String SERVER_TYPE = "FROP RollingSnapshot";
    private final String SNAPSHOT_LOG_FILE = "/ericsson/frh/log/frh_backup/frh_backup.log";
    private final File rollingSnapshotScript = new File("/opt/assuremonitoring-plugins/scripts/frop_rollingsnapshot.pl");
    private final String pluginDescription;
    private final String pluginServerName;
    private final Log log;

    public FROPRollingSnapshotServerDetector() {
        super();
        this.log = getLog();
        this.pluginServerName = "FROP RollingSnapshot";
        this.pluginDescription = "Calculates availability and seconds since last backup for rolling snapshot utility on FROP blade.";
    }
    ...
    public List<ServerResource> getServerResources(final ConfigResponse platformConfig) throws PluginException {
        ...
        if (snapshotlogfile.exists() && snapshotlogfile.canRead() && isLogFilePresent(snapshotlogfile)
            ...
            if (log.isDebugEnabled()) {
                log.debug("This platform does not have a readable FROP Rolling Snapshot Log File (" + snapshotlogfile + ")");
            }
        }
